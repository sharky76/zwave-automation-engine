...
variant_t*  foreach_sep_impl(struct service_method_t* method, va_list args);
...
void evaluate_argument_value(uint32_t key, variant_t* value, void* arg)
{
    ...
    if(NULL != result)
    {
        ...
        service_args_stack_add("Expression.ProcessTemplate", key, result);
    }
}
...
variant_t*  foreach_impl(struct service_method_t* method, va_list args)
{
    ...
    {
        service_args_stack_add("Expression.ProcessTemplate", key, variant_clone(list_item_variant));
